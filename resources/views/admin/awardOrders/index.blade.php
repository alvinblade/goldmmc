        ...
        <div class="content w-100">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">MÜKAFAT ƏMRLƏRİ</h3>
                    <a href="{{ route('admin.awardOrders.create') }}" class="btn btn-primary">
                    ...
                    <div class="block-options">
                        <div class="block-options-item">
                            <div class="d-flex align-items-center justify-content-center">
                                ...
                                <form action="{{route('admin.awardOrders.index')}}" method="GET"
                                      class="d-flex gap-1">
                ...
                <div class="block-content">
                    <table class="js-table-sections table table-sm table-vcenter">
                        ...
                        @foreach ($awardOrders as $order)
                            <tr>
                                <th class="text-center"
                                    scope="row">{{ $awardOrders->firstItem() + $loop->index }}</th>
                                <td class="fs-sm">
                                    {{ $order->order_number }}
                                </td>
                                <td class="fs-sm">
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <a href="{{ route('admin.awardOrders.edit', $order->id) }}"
                                           class="btn btn-sm btn-alt-secondary js-bs-tooltip-enabled"
                                           data-bs-toggle="tooltip" aria-label="Edit"
                                           data-bs-original-title="Edit" title="Düzəliş et">
                                            <i class="fa fa-fw fa-pencil-alt"></i>
                                        </a>
                                        <form action="{{ route('admin.awardOrders.destroy', $order->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    {{ $awardOrders->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </main>
