        ...
        <div class="content w-100">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">MƏZUNİYYƏT ƏMRLƏRİ</h3>
                    <a href="{{ route('admin.defaultHolidayOrders.create') }}" class="btn btn-primary">
                    ...
                    <div class="block-options">
                        <div class="block-options-item">
                            <div class="d-flex align-items-center justify-content-center">
                                ...
                                <form action="{{route('admin.defaultHolidayOrders.index')}}" method="GET"
                                      class="d-flex gap-1">
                ...
                <div class="block-content">
                    <table class="js-table-sections table table-sm table-vcenter">
                        ...
                        @foreach ($defaultHolidayOrders as $order)
                            <tr>
                                <th class="text-center"
                                    scope="row">{{ $defaultHolidayOrders->firstItem() + $loop->index }}</th>
                                <td class="fs-sm">
                                    {{ $order->order_number }}
                                </td>
                                <td class="fs-sm">
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <form action="{{ route('admin.defaultHolidayOrders.destroy', $order->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    {{ $defaultHolidayOrders->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </main>
