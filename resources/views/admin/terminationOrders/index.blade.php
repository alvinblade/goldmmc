        ...
        <div class="content w-100">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">XİTAM ƏMRLƏRİ</h3>
                    <a href="{{ route('admin.terminationOrders.create') }}" class="btn btn-primary">
                    ...
                    <div class="block-options">
                        <div class="block-options-item">
                            <div class="d-flex align-items-center justify-content-center">
                                ...
                                <form action="{{route('admin.terminationOrders.index')}}" method="GET"
                                      class="d-flex gap-1">
                ...
                <div class="block-content">
                    <table class="js-table-sections table table-sm table-vcenter">
                        ...
                        @foreach ($terminationOrders as $order)
                            <tr>
                                <th class="text-center"
                                    scope="row">{{ $terminationOrders->firstItem() + $loop->index }}</th>
                                <td class="fs-sm">
                                    {{ $order->order_number }}
                                </td>
                                <td class="fs-sm">
                                ...
                                <td class="fs-sm">
                                    {{ \Carbon\Carbon::parse($order->termination_date)->format('d.m.Y') }}
                                </td>
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <form action="{{ route('admin.terminationOrders.destroy', $order->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    {{ $terminationOrders->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </main>
