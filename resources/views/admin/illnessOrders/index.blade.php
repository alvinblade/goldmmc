...
@section('title')
    Əmək qabiliyyətinin itirilməsi əmrləri
@endsection
@section('content')
        ...
        <div class="content w-100">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">ƏMƏK QABİLİYYƏTİNİN İTİRİLMƏSİ ƏMRLƏRİ</h3>
                    <a href="{{ route('admin.illnessOrders.create') }}" class="btn btn-primary">
                    ...
                    <div class="block-options">
                        <div class="block-options-item">
                            <div class="d-flex align-items-center justify-content-center">
                                ...
                                <form action="{{route('admin.illnessOrders.index')}}" method="GET"
                                      class="d-flex gap-1">
                ...
                <div class="block-content">
                    <table class="js-table-sections table table-sm table-vcenter">
                        ...
                        @foreach ($illnessOrders as $order)
                            <tr>
                                <th class="text-center"
                                    scope="row">{{ $illnessOrders->firstItem() + $loop->index }}</th>
                                <td class="fs-sm">
                                    {{ $order->order_number }}
                                </td>
                                <td class="fs-sm">
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <form action="{{ route('admin.illnessOrders.destroy', $order->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    {{ $illnessOrders->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </main>
