        ...
        <div class="content w-100">
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">HAMİLƏLİK MƏZUNİYYƏTİ ƏMRLƏRİ</h3>
                    <a href="{{ route('admin.pregnantOrders.create') }}" class="btn btn-primary">
                    ...
                    <div class="block-options">
                        <div class="block-options-item">
                            <div class="d-flex align-items-center justify-content-center">
                                ...
                                <form action="{{route('admin.pregnantOrders.index')}}" method="GET"
                                      class="d-flex gap-1">
                ...
                <div class="block-content">
                    <table class="js-table-sections table table-sm table-vcenter">
                        ...
                        @foreach ($pregnantOrders as $order)
                            <tr>
                                <th class="text-center"
                                    scope="row">{{ $pregnantOrders->firstItem() + $loop->index }}</th>
                                <td class="fs-sm">
                                    {{ $order->order_number }}
                                </td>
                                <td class="fs-sm">
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <form action="{{ route('admin.pregnantOrders.destroy', $order->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    {{ $pregnantOrders->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </main>
