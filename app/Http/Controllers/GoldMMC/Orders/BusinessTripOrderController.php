...
class BusinessTripOrderController extends Controller
{
    public function index(): View|RedirectResponse
    {
        ...
        return view('admin.businessTripOrders.index', compact('businessTripOrders'));
    }
    ...
    public function create(): View|RedirectResponse
    {
        ...
        return view('admin.businessTripOrders.create', compact('employees'));
    }
     ...
     */
    public function store(BusinessTripOrderStoreRequest $request): RedirectResponse
    {
        $companyId = getHeaderCompanyId();
        ...
        if (!$existsAttendanceLog) {
            toast('İşçi tabeldə mövcud deyil', 'errpr');
            return redirect()->back();
        }
            ...
            foreach ($log->days as $k => $day) {
                ...
                if ($dayDate >= $request->start_date && $dayDate <= $request->end_date) {
                    if ($day['status'] == AttendanceLogDayTypes::NULL_DAY->value) {
                        ...
                        toast('Ezamiyyət tarixi aralığı tabel üzrə düzgün qeyd olunmayıb', 'error');
                        return redirect()->back();
                    }
        ...
        $generatedFilePath = returnOrderFile('assets/business_trip_orders/' . $fileName,
            $fileName, 'business_trip_orders');
        ...
        toast('Ezamiyyət əmri uğurla yaradıldı', 'success');

        return redirect()->route('admin.businessTripOrders.index');
    }
    ...
    public function destroy($businessTripOrder): RedirectResponse
    {
        $companyId = getHeaderCompanyId();
        ...
        if (!$businessTripOrder) {
            toast('Ezamiyyət əmri tapılmadı', 'error');
            return redirect()->back();
        }

        foreach (json_decode($businessTripOrder->backup_of_logs, true) as $log) {
        ...
        if (!empty($businessTripOrder->generated_file[0]['path'])) {
            $filePath = public_path($businessTripOrder->generated_file[0]['path']);
        ...
        toast('Ezamiyyət əmri uğurla silindi', 'success');
